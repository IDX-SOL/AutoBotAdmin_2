...
type BotRechargeHistoryPopupProps = {
  open: boolean;
...
export function BotRechargeHistoryPopup({
  open,
  onOpenChange,
  botId,
  botName,
  limit = 100,
}: BotRechargeHistoryPopupProps) {
  ...
  const fetchRecords = useMemo(() => {
    return async () => {
      ...
      setError(null);
      try {
        const response = await adminApiService.getRechargeRecords({
          page: 1,
          limit,
          botId,
          botType: 'volume',
        ...
        console.error('Failed to load bot recharge history', err);
        setError('Unable to load recharge history. Please try again.');
        setRecords([]);
        setSummary(null);
      } finally {
        setLoading(false);
  ...
  const titleLabel = botName?.trim() ? `${botName} (#${botId})` : `Bot #${botId}`;
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={6} className="px-4 py-10 text-center text-gray-500">
                        No recharge records found for this bot.
                      </td>
                    </tr>
                  ) : (
                    records.map((rec) => (
                      <tr key={rec.id} className="hover:bg-gray-800/40">
                        ...
                        <td className="px-4 py-4 font-semibold text-white">{formatAmountCell(rec)}</td>
                        <td className="px-4 py-4 text-gray-300">{rec.currency}</td>
                        <td className="px-4 py-4 text-gray-300">{rec.rechargeType ?? '—'}</td>
                        <td className="px-4 py-4 text-gray-300">{formatPlatformFeeCell(rec)}</td>
